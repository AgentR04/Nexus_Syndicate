...
const SmugglersRun: React.FC = () => {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (e.touches.length > 0 && gameState.isPlaying && isTouching) {
      ...
      setPlayer((prev) => ({
        ...
        y: Math.max(
          0,
          Math.min(gameHeight - prev.height, prev.y + deltaY * 0.8)
        ),
      }));
  ...
  const activateCloak = () => {
    if (
      ...
      gameState.isPaused ||
      gameState.gameOver ||
      gameState.cloakCooldown > 0 ||
      gameState.cloakCharges <= 0
    )
      return;
  ...
  const renderGame = () => {
    ...
    const progressY = 50;
    const progressFill =
      (gameState.distance / gameState.targetDistance) * progressWidth;
  ...
  return (
    <div className="min-h-screen bg-dark-blue p-4 flex flex-col">
      ...
      <div className="flex flex-col lg:flex-row gap-4 flex-grow">
        <div
          id="game-container"
          className="w-full lg:w-3/4 bg-black rounded-lg shadow-neon-blue overflow-hidden relative"
        >
          {/* Game canvas */}
          <canvas
            ref={canvasRef}
          ...
          {!gameState.isPlaying && !gameState.gameOver && (
            <div className="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center bg-black bg-opacity-80 p-4">
              <h2 className="text-2xl md:text-3xl font-cyber text-neon-blue mb-4">
                SMUGGLERS RUN
              </h2>
              ...
              <div className="my-4 text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-white text-sm md:text-base">
                  <div className="bg-dark-blue bg-opacity-50 p-2 rounded">
                    <span className="block font-bold text-neon-blue">
                      DESKTOP
                    </span>
                  ...
                  <div className="bg-dark-blue bg-opacity-50 p-2 rounded">
                    <span className="block font-bold text-neon-green">
                      MOBILE
                    </span>
          ...
          {gameState.gameOver && (
            <div className="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center bg-black bg-opacity-80 p-4">
              <h2
                className={`text-2xl md:text-3xl font-cyber ${
          ...
          {gameState.isPaused && (
            <div className="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center bg-black bg-opacity-80 p-4">
              <h2 className="text-2xl md:text-3xl font-cyber text-neon-yellow mb-4">
                PAUSED
              </h2>
          ...
          {gameState.isPlaying && !gameState.isPaused && !gameState.gameOver && (
            <button
              onClick={togglePause}
              className="absolute bottom-4 right-4 px-4 py-2 bg-neon-yellow bg-opacity-80 text-black font-cyber rounded-md hover:bg-opacity-100 transition-colors flex items-center"
            >
        ...
        <div className="w-full lg:w-1/4 bg-dark-gray rounded-lg p-4 flex flex-col">
          ...
          {/* Game stats */}
          <div className="mb-4">
            <p className="text-neon-green mb-1">
              SCORE: {Math.floor(gameState.score)}
            </p>
            <p className="text-neon-blue mb-1">
              DISTANCE: {Math.floor(gameState.distance)}m
            </p>
            <p className="text-white mb-1">
              TARGET: {Math.floor(gameState.targetDistance)}m
            </p>
            <p className="text-neon-yellow mb-1">
              SPEED: {Math.floor(gameState.speed * 10)}km/h
            </p>
          </div>

          <div className="mb-4">
            <p className="text-neon-purple mb-1">
              ROUTES: {gameState.routesUnlocked}
            </p>
            <p className="text-neon-green mb-1">
              CRYPTO: {gameState.cryptoCrates}
            </p>
          </div>
            ...
            !gameState.gameOver && (
              <div className="mt-auto">
                ...
                <div className="mt-6 bg-dark-blue bg-opacity-30 p-3 rounded border border-neon-blue">
                  <h4 className="text-neon-yellow font-cyber text-sm mb-2">
                    HOW TO PLAY:
                  </h4>
                  <ul className="text-xs text-light-gray space-y-1">
                    <li>
                      • <span className="text-neon-green">Navigate</span> your
                      smuggling drone through hazardous routes
                    </li>
                    <li>
                      • <span className="text-neon-blue">Avoid</span> security
                      drones, firewalls, and data storms
                    </li>
                    <li>
                      • <span className="text-neon-purple">Collect</span>{" "}
                      power-ups to boost speed and activate cloaking
                    </li>
                    <li>
                      • <span className="text-neon-yellow">Gather</span> crypto
                      crates and route keys for rewards
                    </li>
                    <li>
                      • <span className="text-neon-green">Reach</span> the
                      target distance to complete your mission
                    </li>
                  </ul>
                </div>
              </div>
