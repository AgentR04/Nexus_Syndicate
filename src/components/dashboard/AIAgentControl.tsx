...
// Mock agent types
const agentTypes = [
  { id: 'scout', name: 'Scout', description: 'Gathers intelligence and explores new territories', cost: 500, maintenanceCost: 50 },
  { id: 'defender', name: 'Defender', description: 'Protects territories from enemy attacks', cost: 750, maintenanceCost: 75 },
  { id: 'trader', name: 'Trader', description: 'Automates market transactions and finds arbitrage opportunities', cost: 1000, maintenanceCost: 100 },
  { id: 'hacker', name: 'Hacker', description: 'Infiltrates enemy systems and steals resources', cost: 1250, maintenanceCost: 125 },
  { id: 'researcher', name: 'Researcher', description: 'Accelerates technology research and development', cost: 1500, maintenanceCost: 150 },
];
...
const mockTasks = [
  ...
  { id: 'research', name: 'Accelerate Research', compatibleTypes: ['researcher'] },
  { id: 'patrol', name: 'Patrol Area', compatibleTypes: ['scout', 'defender'] },
  { id: 'sabotage', name: 'Sabotage Enemy', compatibleTypes: ['hacker', 'scout'] },
  { id: 'arbitrage', name: 'Find Arbitrage', compatibleTypes: ['trader'] },
];

const AIAgentControl: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'deployed' | 'create' | 'upgrade'>('deployed');
  ...
  const handleDeployAgent = (e: React.FormEvent) => {
    ...
    alert(`Agent ${newAgentName} of type ${selectedAgentType} deployed to ${mockTerritories.find(t => t.id === selectedTerritory)?.name} with task ${mockTasks.find(t => t.id === selectedTask)?.name}`);
    setNewAgentName('');
  ...
  return (
    <div className="cyber-panel p-4">
      <h2 className="text-xl font-cyber text-neon-blue mb-4">AI AGENT CONTROL</h2>
      
      {/* Tabs */}
      <div className="flex border-b border-neon-blue mb-4">
        <button
          className={`px-4 py-2 font-cyber text-sm ${activeTab === 'deployed' ? 'text-neon-blue border-b-2 border-neon-blue' : 'text-light-gray'}`}
          onClick={() => setActiveTab('deployed')}
        >
          DEPLOYED
        </button>
        <button
          className={`px-4 py-2 font-cyber text-sm ${activeTab === 'create' ? 'text-neon-blue border-b-2 border-neon-blue' : 'text-light-gray'}`}
          onClick={() => setActiveTab('create')}
        >
          CREATE
        </button>
        <button
          className={`px-4 py-2 font-cyber text-sm ${activeTab === 'upgrade' ? 'text-neon-blue border-b-2 border-neon-blue' : 'text-light-gray'}`}
      ...
      {activeTab === 'deployed' && (
        <div>
            ...
            {mockDeployedAgents.map((agent) => (
              <div 
                ...
                className={`bg-dark-gray p-3 rounded cursor-pointer transition-colors ${
                  selectedAgent === agent.id ? 'border border-neon-blue' : 'hover:bg-medium-gray'
                }`}
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <div className="flex items-center">
                      <div className={`w-2 h-2 rounded-full ${agent.status === 'active' ? 'bg-neon-green' : 'bg-neon-yellow'} mr-2`}></div>
                      <h4 className={`font-cyber ${getAgentTypeColor(agent.type)}`}>{agent.name}</h4>
                      <span className="ml-2 text-xs text-light-gray">Lvl {agent.level}</span>
                    </div>
                  ...
                  <div className="text-right">
                    <div className={`text-sm ${getPerformanceColor(agent.performance)}`}>
                      {agent.performance}%
                    </div>
                ...
                {selectedAgent === agent.id && (
                  <div className="mt-3 pt-3 border-t border-neon-blue border-opacity-30">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <div className="text-xs text-light-gray mb-1">Health</div>
                      ...
                      <div>
                        <div className="text-xs text-light-gray mb-1">Last Report</div>
          ...
          <div className="mt-4 p-3 bg-dark-gray rounded">
            <h3 className="text-sm font-cyber text-neon-blue mb-2">AGENT NETWORK PERFORMANCE</h3>
      ...
      {activeTab === 'create' && (
        <div>
          <h3 className="text-sm font-cyber text-neon-purple mb-3">CREATE NEW AGENT</h3>
          
          <form onSubmit={handleDeployAgent}>
            <div className="mb-4">
              <label className="block text-xs text-light-gray mb-1">AGENT TYPE</label>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {agentTypes.map((type) => (
                  ...
                  >
                    <div className={`text-sm font-cyber ${getAgentTypeColor(type.id)}`}>{type.name}</div>
                    <div className="text-xs text-light-gray mt-1">Cost: {type.cost}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {selectedAgentType && (
              <>
                <div className="mb-4">
                  <label className="block text-xs text-light-gray mb-1">AGENT NAME</label>
                  <input
                    type="text"
                    value={newAgentName}
                    ...
                    placeholder={`${agentTypes.find(t => t.id === selectedAgentType)?.name}-${Math.floor(Math.random() * 100)}`}
                    required
                  />
                </div>
                
                <div className="mb-4">
                  <label className="block text-xs text-light-gray mb-1">DEPLOYMENT LOCATION</label>
                  <div className="grid grid-cols-2 gap-2">
                    {mockTerritories.map((territory) => (
                      ...
                      >
                        ...
                        <div className="text-xs text-light-gray mt-1">{territory.type}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="mb-4">
                  <label className="block text-xs text-light-gray mb-1">INITIAL TASK</label>
                  <div className="grid grid-cols-2 gap-2">
                    {mockTasks
                      .filter(task => task.compatibleTypes.includes(selectedAgentType))
                      .map((task) => (
                        <div 
                          key={task.id}
                          className={`p-2 rounded cursor-pointer transition-colors ${
                ...
                <div className="mb-4">
                  <h3 className="text-sm font-cyber text-neon-purple mb-2">DEPLOYMENT SUMMARY</h3>
                  <div className="bg-dark-gray p-3 rounded">
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <div className="text-xs text-light-gray">Agent Type</div>
                        <div className="text-sm">{agentTypes.find(t => t.id === selectedAgentType)?.name}</div>
                      </div>
                      <div>
                        <div className="text-xs text-light-gray">Initial Cost</div>
                        <div className="text-sm">{agentTypes.find(t => t.id === selectedAgentType)?.cost} Credits</div>
                      </div>
                      <div>
                        <div className="text-xs text-light-gray">Maintenance</div>
                        <div className="text-sm">{agentTypes.find(t => t.id === selectedAgentType)?.maintenanceCost}/hr</div>
                      </div>
                      <div>
                        <div className="text-xs text-light-gray">Deployment Time</div>
                ...
                <button
                  type="submit"
                  disabled={!selectedAgentType || !selectedTerritory || !selectedTask}
      ...
      {activeTab === 'upgrade' && (
        <div>
          <h3 className="text-sm font-cyber text-neon-purple mb-3">UPGRADE AGENTS</h3>
              ...
              <div key={agent.id} className="bg-dark-gray p-3 rounded">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <div className="text-xs text-light-gray mt-1">
                      {agent.type.charAt(0).toUpperCase() + agent.type.slice(1)} Type
                    </div>
                  </div>
                  <div>
                    <button 
                      ...
                      onClick={() => alert(`Upgrading ${agent.name} to level ${agent.level + 1}`)}
                    >
                ...
                <div className="mt-3 grid grid-cols-2 gap-3">
                  <div>
                    <div className="text-xs text-light-gray mb-1">Current Level: {agent.level}</div>
                    <div className="w-full bg-dark-blue rounded-full h-2">
                      <div 
                  ...
                  <div>
                    <div className="text-xs text-light-gray mb-1">Next Level: {agent.level + 1}</div>
          ...
          <div className="mt-4 p-3 bg-dark-blue rounded">
            <h3 className="text-sm font-cyber text-neon-yellow mb-2">UPGRADE BENEFITS</h3>
            <div className="text-xs text-light-gray space-y-1">
              ...
              <p>• Enhanced territory defense capabilities</p>
              <p>• Better market analysis and trading algorithms</p>
            </div>
          </div>
        </div>
      )}
