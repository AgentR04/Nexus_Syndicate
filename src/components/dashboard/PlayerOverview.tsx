...
const PlayerOverview: React.FC<PlayerOverviewProps> = ({ playerData, walletAddress }) => {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'attack':
        return (
          <svg className="w-4 h-4 text-neon-pink" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      default:
        return (
          <svg className="w-4 h-4 text-neon-yellow" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div className="cyber-panel p-4 relative overflow-hidden">
      ...
      <h2 className="text-xl font-cyber text-neon-blue mb-4">PLAYER OVERVIEW</h2>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
        ...
        <div className="flex items-center mb-4 md:mb-0">
          <div className="relative">
            <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-neon-purple">
              <img 
                src={'https://via.placeholder.com/150/1a1a2e/00ffff?text=NS'} 
                alt={playerData.name}
                className="w-full h-full object-cover"
                onError={(e) => {
                  const target = e.target as HTMLImageElement;
                  target.src = 'https://via.placeholder.com/150/1a1a2e/00ffff?text=NS';
                }}
              />
            </div>
          ...
          <div className="ml-4">
            <h3 className="text-lg font-cyber text-neon-purple">{playerData.name}</h3>
            <div className="flex items-center">
              <span className="text-xs text-light-gray">{playerData.faction}</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 w-full md:w-auto">
          <div className="cyber-resource-box">
            ...
            <div className="text-neon-blue font-mono">{formatNumber(playerData.credits)}</div>
          </div>
          <div className="cyber-resource-box">
            <div className="text-xs text-light-gray">DATA SHARDS</div>
            <div className="text-neon-green font-mono">{formatNumber(playerData.dataShards)}</div>
          </div>
          <div className="cyber-resource-box">
            <div className="text-xs text-light-gray">INFLUENCE</div>
            <div className="text-neon-purple font-mono">{formatNumber(playerData.influence)}</div>
          </div>
          <div className="cyber-resource-box">
            <div className="text-xs text-light-gray">TERRITORIES</div>
            <div className="text-neon-pink font-mono">{formatNumber(playerData.territories)}</div>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-64 cyber-panel z-10">
              <div className="p-2">
                <h4 className="text-sm font-cyber text-neon-blue mb-2">NOTIFICATIONS</h4>
                {playerData.notifications.length > 0 ? (
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {playerData.notifications.map(notification => (
                      <div key={notification.id} className="flex p-2 bg-dark-blue bg-opacity-50 rounded">
                        <div className="mr-2 mt-1">
                          {getNotificationIcon(notification.type)}
                        </div>
                        <div>
                          <div className="text-xs">{notification.message}</div>
                          <div className="text-xs text-light-gray opacity-70">{notification.time}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-xs text-light-gray">No new notifications</div>
                )}
              </div>
            </div>
          )}
      ...
      <div className="mt-4 w-full">
        <div className="flex justify-between text-xs mb-1">
          ...
          <span className="font-mono text-neon-blue">{playerData.currentXP} / {playerData.nextLevelXP}</span>
        </div>
        <div className="h-2 bg-dark-gray rounded-full overflow-hidden">
          <div 
            className="h-full bg-gradient-to-r from-neon-blue to-neon-purple" 
            style={{ width: `${(playerData.currentXP / playerData.nextLevelXP) * 100}%` }}
          ></div>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        ...
        <div className="cyber-stat-box">
          <div className="text-xs text-light-gray">TERRITORIES</div>
          <div className="text-neon-green font-mono">{playerData.territories}</div>
        </div>
      </div>
    </div>
  );
